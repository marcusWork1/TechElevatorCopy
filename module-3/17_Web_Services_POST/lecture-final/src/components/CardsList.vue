...
export default {
  ...
  methods: {
    // this methods retrieves from teh API server resource (API call in services)
    //   and stores in the data store (using Mutations in data store)
    retrieveCards() {
      boardsService
        .getCards(this.boardId)  // Get the data from API resource using service method
        .then(response => {
          this.title = response.data.title;
          this.$store.commit("SET_BOARD_CARDS", response.data.cards);  // and store it in the data stores
