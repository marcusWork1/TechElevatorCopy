...
public class USCitiesAndParksCLI {
    ...
    public static void main(String[] args) {
        ...
        // The database the program wants to use
        BasicDataSource dataSource = new BasicDataSource();
        //                url for postgres UnitedState database
        //                type:DBMS-Name   // server  :port/database
        dataSource.setUrl("jdbc:postgresql://localhost:5432/UnitedStates");
        dataSource.setUsername("postgres");  // owner of the database - NOT REAL WORLD
        dataSource.setPassword("postgres1"); // database password - NOT REAL WORLD
    ...
    public USCitiesAndParksCLI(DataSource dataSource) {
        stateDao = new JdbcStateDao(dataSource);  // Instantiate a DAO with the datasource
        cityDao = new JdbcCityDao(dataSource);    // Instantiate a DAO with the datasource
        parkDao = new JdbcParkDao(dataSource);    // Instantiate a DAO with the datasource
    }
