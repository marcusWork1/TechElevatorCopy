...
// This will define the components required for accessing the database manager
//
// Establish and define values related to the database manager and database to be used
// Every test will use this information to connect to the database before the test
//
// What this does:
//  1. Defines and connects to the database manager
//  2. It creates a database for testing - never use real, production/customer data in tests
//                                         you want control of the data being used in the test
//                                         don't want to accidentally change/affect real data
//                                         privacy laws
//  3. Create tables and possibly populate them (also done in each test)
//  4. Rollback changes when tests are done
//  5. Drop the test database and disconnect from Database Manager when tests done
...
public class TestingDatabaseConfig {
    ...
    /* Every test will have this after it's constructor is run             */
    @PostConstruct
    public void setup() {
    ...
    public DataSource dataSource() throws SQLException {
         ...
         *
         * Lets the test/testers control the rollback
         * */
        dataSource.setAutoCommit(false); // may be required for all database managers
    ...
    *                     and disconnect from database manager
    */
    @PreDestroy
    public void cleanup() {
