...
2. Populate the `campground` database using the `database/campground.sql` file.
3. Verify the tables have been created correctly by oerforming the tasks in ***Step Two***.
...
Open `step-3.sql`. Each SQL statement you need to write starts with a comment describing the requirements. In this step, you'll write 16 queries.  Be sure you are connected to the `campground` database.

> Note: If you need to select all columns from a table, avoid using `select *` in favor of column names.  Use of `select *` may cause issues when the SQL is used from a program.

## Step Four: Query from multiple tables

Open `step-4.sql`. Each SQL statement you need to write starts with a comment describing the requirements. In this step, you'll write six queries. Be sure you are connected to the `campground` database.

## Step Five: Add, modify, and remove data from a database

Open `step-5.sql`. You need to add a new park, campground, sites, and reservations to the National Park Database. Each SQL statement you need to write starts with a comment describing the requirements. In this step, you'll write seven queries. Be sure you are connected to the `campground` database.
